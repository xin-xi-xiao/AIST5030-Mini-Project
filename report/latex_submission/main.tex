...
\textbf{Subject-level ablation reading}: backpack and dog show consistent
co-optimization under E3 (best CLIP-I and best DINO simultaneously), while cat
reveals an objective tension where E2 maximizes identity fidelity but E3
maximizes semantic similarity. This asymmetry suggests that practical model
selection should be target-aware: identity-centric deployments prefer higher
CLIP-I settings, whereas semantic-consistency-sensitive applications prefer
higher DINO settings.

\textbf{Training-process implication}: because all methods are run with the same
prompt set, step budget, and data split, the observed cross-method differences
are attributable to parameterization behavior rather than experimental
confounders. This strengthens the causal interpretation of OFT-vs-LoRA and
OFT-vs-BOFT comparisons in this report.
...
These checks reduce the risk of silent pipeline faults (partial outputs,
malformed metrics, or serialization drift), and support using the observed
method ranking for downstream conclusions.
...
\begin{center}
\includegraphics[width=0.82\columnwidth]{assets/qual/qual_all_subjects.png}
